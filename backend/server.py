...
from orderry_service import create_orderry_lead  # 👈 NEW IMPORT
...
async def create_booking(booking_data: BookingCreate):
    ...
    # 👇 NEW: Send to Orderry as Lead
    def send_to_orderry():
        try:
            create_orderry_lead(
                name=booking_data.customerName,
                phone=booking_data.phone,
                email=booking_data.email or "",
                message=f"Package: {package['name']} | Date: {booking_data.travelDate} | Guests: {booking_data.guests} | {booking_data.message or ''}"
            )
            logger.info("Orderry lead created for booking")
        except Exception as e:
            logger.error(f"Orderry lead failed: {e}")

    threading.Thread(target=send_to_orderry, daemon=True).start()
...
async def create_contact(contact_data: ContactCreate):
    ...
    # 👇 NEW: Send Contact to Orderry as Lead
    def send_contact_to_orderry():
        try:
            create_orderry_lead(
                name=contact_data.name,
                phone=contact_data.phone,
                email=contact_data.email or "",
                message=f"Contact Form: {contact_data.message or ''}"
            )
            logger.info("Orderry lead created for contact")
        except Exception as e:
            logger.error(f"Orderry contact lead failed: {e}")

    threading.Thread(target=send_contact_to_orderry, daemon=True).start()
